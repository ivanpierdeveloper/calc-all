...
async function verifyKeyUpBin() {
    ...
    if(event.keyCode != 48 && event.keyCode != 49 && event.keyCode != 96 && event.keyCode != 97 && event.keyCode != 8 && event.keyCode != 13) {
        cn.info("Non hai premuto uno di questi tasti 48 49 96 97 8 13");
    ...
    // quandoi premo il tasto invio
    if(event.keyCode == 13) {
        if(valueInputBin.value != "") {
            calcoloBin();
        } else {
            fnc.showMyAlert("AVVISO", "campo binario vuoto o valore non corretto.", 'var(--warning)', 'var(--danger)', 'var(--dark)', 'var(--dark)');
        }
...
async function verifyKeyUpHex() {
    ...
    if(event.keyCode == 13) {
        if(val_hex != "") {
            ...
            fnc.showMyAlert("AVVISO", "campo hex vuoto o valore non corretto.", 'var(--warning)', 'var(--danger)', 'var(--dark)', 'var(--dark)');
        }
    }// ./quando premo il tasto invio
}// ./// verifyKeyUpHex
// verifyKeyUpDec
async function verifyKeyUpDec() {
    ...
    if(event.keyCode == 13) {
        if(val_dec != "") {
            ...
            fnc.showMyAlert("AVVISO", "campo decimale vuoto o valore non corretto.", 'var(--warning)', 'var(--danger)', 'var(--dark)', 'var(--dark)');
        }
    }// ./quando premo il tasto invio
}// ./// verifyKeyUpDec
// calcoloDec
